...
function draw(ctx, seed) {
  ...
  class Cell {
    constructor(i, j, seed2) {
      ...
      let item = i % 4 + j % 4 * 4;
      let mode = bits(seed2, segment * 2, segment * 2 + 2);
      this.pref = pref(patterns[mode][segment][item]);
    }
    neighbors(cells2) {
      let nbrs = [null, null, null, null];
      if (this.j > 0)
        nbrs[0] = cells2[this.i][this.j - 1];
      if (this.i < cells2.length - 1)
        nbrs[1] = cells2[this.i + 1][this.j];
      if (this.j < cells2.length - 1)
        nbrs[2] = cells2[this.i][this.j + 1];
      if (this.i > 0)
        nbrs[3] = cells2[this.i - 1][this.j];
      return nbrs;
    }
    neighbor(cells2) {
      let nbrs = this.neighbors(cells2);
    ...
    paths(cells2) {
      return this.neighbors(cells2).filter((x, i) => x && !this.walls[i]);
    }
    faces(ctx2, inc2) {
      if (this.inMaze)
    ...
    blob(ctx2, inc2) {
      ctx2.beginPath();
      ctx2.arc(this.i * inc2 + inc2 / 2, this.j * inc2 + inc2 / 2, inc2 / 3, 0, 2 * Math.PI);
    ...
    edges(ctx2, inc2) {
      ctx2.beginPath();
      if (this.walls[0]) {
        ctx2.moveTo(this.i * inc2, this.j * inc2);
        ctx2.lineTo(this.i * inc2 + inc2, this.j * inc2);
      }
      if (this.walls[1]) {
        ctx2.moveTo(this.i * inc2 + inc2, this.j * inc2);
        ctx2.lineTo(this.i * inc2 + inc2, this.j * inc2 + inc2);
      }
      if (this.walls[2]) {
        ctx2.moveTo(this.i * inc2 + inc2, this.j * inc2 + inc2);
        ctx2.lineTo(this.i * inc2, this.j * inc2 + inc2);
      }
      if (this.walls[3]) {
        ctx2.moveTo(this.i * inc2, this.j * inc2 + inc2);
        ctx2.lineTo(this.i * inc2, this.j * inc2);
      }
      ctx2.stroke();
    }
  }
  ...
  function deadend(cell, cells2, last) {
    let ns2 = cell.paths(cells2);
    let m = cells2.length - 1;
    if (ns2.length == 1) {
      return cell;
    } else if (ns2.length == 2) {
      let next = ns2.find((x) => x != last);
      if (next.i == 0 || next.i == m || next.j == 0 || next.j == m) {
        return deadend(next, cells2, cell);
      }
    }
    return null;
  }
  ...
  ctx.lineWidth = inc / 4;
  ctx.lineCap = "square";
  for (let step = 0;step < 1e5; ++step) {
    if (walk.length == 0) {
      let start2 = null;
      for (let i = 0;i < cols && !start2; i++)
        for (let j = 0;j < rows && !start2; j++)
          if (!cells[i][j].inMaze && !cells[i][j].walk)
            start2 = [i, j];
      if (start2) {
        walk.push(cells[start2[0]][start2[1]]);
        cells[start2[0]][start2[1]].walk = true;
      } else {
        break;
      }
    } else {
